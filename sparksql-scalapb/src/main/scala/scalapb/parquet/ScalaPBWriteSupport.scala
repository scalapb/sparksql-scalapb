...
class ScalaPBWriteSupport[T <: GeneratedMessage with Message[T]](
    implicit ct: reflect.ClassTag[T] = null
) extends WriteSupport[T] {
  var pbClass: Class[T] =
    Option(ct).map(_.runtimeClass.asInstanceOf[Class[T]]).orNull
  var recordConsumer: RecordConsumer = null

  override def init(configuration: Configuration): WriteContext = {
    if (pbClass == null) {
      pbClass = configuration
        .getClass(
          ScalaPBWriteSupport.SCALAPB_CLASS_WRITE,
          null,
          classOf[GeneratedMessage]
        )
        .asInstanceOf[Class[T]]
      if (pbClass == null) {
        throw new BadConfigurationException(
          "ScalaPB class not specified. Please use ScalaPBOutputFormat.setMessageClass."
        )
      }
    }
    val descriptor: Descriptor =
      pbClass.getMethod("descriptor").invoke(null).asInstanceOf[Descriptor]
...
object ScalaPBWriteSupport {
  ...
  def setSchema[T <: GeneratedMessage](
